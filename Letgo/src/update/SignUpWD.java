...
class SignUpWD implements ActionListener , KeyListener {
	...
	public void actionPerformed(ActionEvent e) {		
		if (e.getSource() == buttons.get("회원가입") ) {
			...
			//폴더위치
			String folderPath = "."+File.separator+ "member"+File.separator+textFields.get("아이디").getText();
			//id.txt 위치
			...
			if (!labels.get("아이디체크").getText().equals("")
				...
				labels.get("보물체크").setText("오류를 확인하세요");
			} else {
				...
				try {
					...
					byte f[] = (textFields.get("생년").getText()+comboBoxs.get("생월").getSelectedItem()+textFields.get("생일").getText()).getBytes();
					birth.write(f);	
	...
	public void keyReleased(KeyEvent e) {
		...
		if (textFields.get("아이디").getText().equals("")) {
			...
			if (textFields.get("아이디").getText().length() <5 || textFields.get("아이디").getText().length() > 20) {
	...
	void arrangement(){
		...
		setLabels("아이디설명", "*5~20자의 영문 소문자, 숫자만 사용가능합니다", standardX, standardY + (HeightY + interval)*n++, 400, HeightY);
		...
		setLabels("비밀번호설명", "*8~16자 영문 대 소문자, 숫자만 사용가능합니다", standardX, standardY + (HeightY + interval)*n++, 400, HeightY);
		...
		setLabels("생년월일", "생년월일", standardX, standardY + (HeightY + interval)*n, 300, HeightY);
