...
public class IdFindWD extends JFrame  implements ActionListener { 
	...
	void setSText() {
		...
		add(jp2,BorderLayout.WEST);  // 자리지정
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==jb1) {
			...
			//String path = ; // 멤버까지의 주소 가져오고 /로 바꿔줌.
			...
			for(int i=0; i<filenames.length;i++) {
				...
				String idPath = "member/"+filenames[i].getName()+"/id.txt";  // id의 텍스트 경로
				...
				if(na.exists()) {
					...
					try {  // f : 이름, d: 답, w : id출력
						...
						String data1 = new String(b);			// b1에 쌓인 내용을 data1에 저장.
						...
						if(data1.equals(text1.getText()) && data2.equals(text2.getText())) {  // equals가 String변수 비교해줌.
							// 비밀번호 검토 및 확인
							FileInputStream f2 = new FileInputStream(id);
							label5.setText(new String(o));

						} else if (String.valueOf(text1.getText()).equals("") || text2.getText().equals("")) {
