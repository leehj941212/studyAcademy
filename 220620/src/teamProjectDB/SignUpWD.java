	...
	public void actionPerformed(ActionEvent e) {	
		...
		if (e.getSource() == buttons.get("회원가입") ) {
			...
			if (!checkLabels.get("아이디체크").getText().equals("")
				...
				student.setName(textFields.get("이름").getText());
				student.setCallNum(textFields.get("휴대폰번호").getText());
				student.setId(textFields.get("아이디").getText());
				student.setPassword(String.valueOf(passwordFields.get("비밀번호").getPassword()));
				student.setQuestions(textFields.get("보물").getText());
				student.setGender(comboBoxs.get("성별").getSelectedItem().toString());
				student.setBirth(Integer.parseInt(textFields.get("생년").getText() + comboBoxs.get("생월").getSelectedItem().toString() + (textFields.get("생일").getText())));
				student.setHakbun(Integer.parseInt(textFields.get("학번").getText()));
				student.setJava(0);
				student.setWeb(0);
				student.setJsp(0);
			...
			if (textFields.get("아이디").getText().length() <2 || textFields.get("아이디").getText().length() > 20) {
				...
				for (int i = 0; i < idCheck.length; i++) {
					if (!Character.isLowerCase(idCheck[i]) && !Character.isDigit(idCheck[i])) {
						...
						if (act.idLookUp(textFields.get("아이디").getText())) {
			...
			if (textFields.get("아이디").getText().length() <2 || textFields.get("아이디").getText().length() > 20) {
				...
				for (int i = 0; i < idCheck.length; i++) {
					...
					if (!Character.isLowerCase(idCheck[i]) && !Character.isDigit(idCheck[i])) {
						...
						break;
						//아이디가 중복되었을때
					} else if (act.idLookUp(textFields.get("아이디").getText())) {
						checkLabels.get("아이디체크").setText("중복된 아이디 입니다");
						//문제없을때
					} else {
						checkLabels.get("아이디체크").setText("");
